...
def create_moodle_xml(docx_path, output_path):
    ...
    for block in doc.element.body:
        ...
        if block.tag.endswith('p'):
            ...
            if current_soal is not None:
                # Deteksi Isi Soal
                if text.lower().startswith('isi soal nomor'):
                    clean_text = re.sub(r'(?i)isi soal nomor \d+', '', text).strip()
                    current_soal['html'] += f"<p>{clean_text}</p>"
                ...
                # Deteksi Opsi
                elif text.lower().startswith('[opsi'):
                    opt_content = re.sub(r'(?i)\[opsi [A-E]\]', '', text).strip()
                    current_soal['options'].append(opt_content)
                
                # Deteksi Kunci Jawaban
                elif text.lower().startswith('[kunci:'):
                    key_raw = re.search(r'(?i)\[KUNCI:\s*(.*)\]', text)
                    if key_raw:
    ...
    for item in questions:
        ...
        final_html = item['html'].replace("2x2", "2x^2").replace("cos2x", "\\cos^2 x")
        ET.SubElement(qtext_node, "text").text = f"<![CDATA[{final_html}]]>"
